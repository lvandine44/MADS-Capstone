...
# # Initialize Flask app and specify the path for the templates folder
...
# # Endpoint to get all restaurant names (menu items)
# @app.route('/get-restaurants', methods=['GET'])
# def get_restaurants():
#     try:
#         # Get the list of restaurant names from the menu dataset
...
#     try:
#         # Get the list of food names from the food dataset
#         food_names = food_dataset["Food_Name"].tolist()
#         return jsonify({"food_items": food_names})
#     except Exception as e:
#         return jsonify({"error": str(e)}), 400
    
# # Route to fetch food items by restaurant
...
#         if len(foods) == 0:
#             return jsonify({"error": f"No foods found for restaurant '{restaurant_name}'"}), 404
